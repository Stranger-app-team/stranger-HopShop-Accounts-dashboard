...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        const user = data.data?.hopUser || data.hopUser;
        const token = data.data?.token || data.token;
        const normalizedRole = user?.role?.trim().toUpperCase();
        console.log('Normalized user role:', normalizedRole);

        const allowedRoles = ['HT', 'VENDOR', 'AT'];

        if (user && token && allowedRoles.includes(normalizedRole)) {
          localStorage.setItem('authToken', token);
          localStorage.setItem('userData', JSON.stringify(user));

        if (normalizedRole === 'HT') {
  router.push('/orders');
} else if (normalizedRole === 'VENDOR') {
  router.push('/authenticated/dashboard');
} else if (normalizedRole === 'AT') {
  router.push('/authenticated/dashboard'); 
}
        } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 text-black via-white to-blue-50 flex items-center justify-center p-4">
